...
namespace FileConverter.Services
{
    ...
    public partial class SettingsService : ObservableObject, ISettingsService
    {
        ...
        public bool PostInstallationInitialization()
        {
            ...
            // Auto-import presets selected by user during installation from Presets folder
            try
            {
                string presetsDirectory = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Presets");
                if (Directory.Exists(presetsDirectory))
                {
                    ...
                    foreach (string presetFile in presetFiles)
                    {
                        ...
                        // Skip the full backup XML to keep active preset list clean unless manually imported
                        if (string.Equals(fileName, "All_Default_Presets.xml", StringComparison.OrdinalIgnoreCase))
                        {
                            continue;
                        }

                        List<ConversionPreset> presets = this.LoadPresetsFromFile(presetFile);
                        foreach (ConversionPreset preset in presets)
                        {
                            if (!defaultSettings.ConversionPresets.Any(p => string.Equals(p.FullName, preset.FullName, StringComparison.OrdinalIgnoreCase)))
                            {
                                defaultSettings.ConversionPresets.Add(preset);
                            }

                            // Register custom input types
                            if (preset.InputTypes != null)
                            {
                                foreach (string inputType in preset.InputTypes)
                                {
                                    if (!Helpers.CompatibleInputExtensions.Contains(inputType, StringComparer.OrdinalIgnoreCase) &&
                                        !defaultSettings.CustomInputExtensions.Contains(inputType, StringComparer.OrdinalIgnoreCase))
                                    {
                                        defaultSettings.CustomInputExtensions.Add(inputType);
                                    }
                                }
                            }
                        }
            ...
            {
                Debug.LogError($"Error while auto-importing installed presets: {ex.Message}");
            }
        ...
        private List<ConversionPreset> LoadPresetsFromFile(string filePath)
        {
            List<ConversionPreset> presets = new List<ConversionPreset>();
            try
            {
                ...
                {
                    XmlHelpers.LoadFromFile<ConversionPreset>(rootElementName, filePath, out ConversionPreset singlePreset);
                ...
                else
                {
                    ...
                    XmlHelpers.LoadFromFile<List<ConversionPreset>>(rootToUse, filePath, out presets);
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Failed to load presets from {filePath}: {ex.Message}");
            }

            return presets ?? new List<ConversionPreset>();
        }
    }
}
