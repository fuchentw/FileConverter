...
namespace FileConverter
{
    ...
    public class Settings : ObservableObject, IXmlSerializable
    {
        ...
        private ObservableCollection<string> customInputExtensions = new ObservableCollection<string>();
        ...
        public Settings Merge(Settings settings)
        {
            ...
            if (settings.customInputExtensions != null)
            {
                foreach (string ext in settings.customInputExtensions)
                {
                    if (!this.customInputExtensions.Contains(ext))
                    {
                        this.customInputExtensions.Add(ext);
                    }
                }
            }
        ...
        [XmlElement("CustomInputExtension")]
        public string[] SerializableCustomInputExtensions
        {
            get => this.customInputExtensions.ToArray();
            set
            {
                this.customInputExtensions.Clear();
                if (value != null)
                {
                    for (int index = 0; index < value.Length; index++)
                    {
                        string ext = value[index]?.Trim().TrimStart('.').ToLowerInvariant();
                        if (!string.IsNullOrEmpty(ext) && !this.customInputExtensions.Contains(ext))
                        {
                            this.customInputExtensions.Add(ext);
                        }
                    }
        ...
        [XmlIgnore]
        public ObservableCollection<string> CustomInputExtensions => this.customInputExtensions;
        ...
        public CultureInfo ApplicationLanguage
        {
            ...
            set
            {
                ...
                if (this.applicationLanguage != null)
                {
                    ...
                    FileConverter.Properties.Resources.Culture = this.applicationLanguage;
                }
